...
# Un [[wikilink]] scris în interiorul unui bloc de cod sau al unui cod inline
# NU e o legătură — Obsidian nu îl interpretează. Notele care documentează
# convenția vault-ului conțin literalul `[[wikilink]]` exact așa, ca exemplu,
# și ar fi raportate greșit ca legături rupte.
BLOC_COD = re.compile(r"```.*?```", re.S)
COD_INLINE = re.compile(r"`[^`\n]*`")


def fara_cod(text):
    return COD_INLINE.sub("", BLOC_COD.sub("", text))
...
def main():
    ...
    for nume, text in note.items():
        for tinta in LINK.findall(fara_cod(text)):
            tinta = tinta.strip()
            total_linkuri += 1
